    ...
    getFilter = stateMenus => {
        ...
        if (!search) {
            ...
            return menus.filter(function(menu) {
                const filterMenu =
                    menu.type === 'link' || menu.type === 'function' ? menu.name.toLowerCase().search(search.toLowerCase()) !== -1 : menu;

                if (filterMenu.type === 'folder') {
                    filterMenu.children = filterMenu.children.filter(function(child) {
                        return child.name.toLowerCase().search(search.toLowerCase()) !== -1;
                    });
                }
                return filterMenu;
            });
                            ...
                            <List component="nav" className={this.props.classes.listMenu}>
                                {menu.type === 'folder' && menu.children <= 0 ? (
                                    ...
                                    <ListItem
                                        ...
                                        className={
                                            (menu.type.toLowerCase() === 'link' || menu.type.toLowerCase() === 'function') &&
                                            menu.parent_id !== undefined
                                                ? this.props.classes.nestedMenu
                                                : this.props.classes.defaultMenu
                                        }
                                    >
                                        <ListItemIcon>
                                        ...
                                        <ListItemText
                                            style={this.state.hideMenu ? { display: 'none' } : { display: 'block', padding: 0 }}
                                            classes={{ primary: this.props.classes.listLabel }}
                                            inset={false}
                                            primary={menu.name}
                                        />
                                            ...
                                            <div style={this.state.active === menu.id ? { marginRight: 0 } : { marginRight: 4 }}>
                                                {menu.type.toLowerCase() === 'folder' ? (
                                                    this.state['open' + menu.id] ? (
                                                        <Icon style={{ color: this.props.theme.palette.primary.main, fontSize: 22 }}>
                                                            expand_less
                                                        </Icon>
                                                    ) : (
                                                        <Icon style={{ color: this.props.theme.palette.primary.main, fontSize: 22 }}>
                                ...
                                <Collapse
                                    style={this.state.hideMenu ? { display: 'none' } : { display: 'block' }}
                                    in={this.state['open' + menu.id]}
                                    timeout="auto"
                                    unmountOnExit
                                >
    ...
    render() {
        ...
        return (
            <div className={classes.root}>
                ...
                <Drawer
                    style={this.state.hideMenu ? { width: 55 } : { width: drawerWidth }}
                    variant="permanent"
                    classes={{
                ...
                >
                    ...
                    <div
                        ...
                        style={{
                            backgroundImage:
                                this.state.user.profile_image && this.state.user.profile_image.public_url
                                    ? 'url("' + this.state.user.profile_image.public_url + '")'
                                    : 'url("/res/info.png")',
                        }}
                    >
                        <div
                            className={classes.welcomeBackground}
                            style={this.state.hideMenu ? { padding: theme.spacing(3) } : { padding: theme.spacing(3) }}
                        >
                            <Grid container spacing={6} alignItems="center" direction="row" justify="space-between">
                                <div style={this.state.hideMenu ? { display: 'none' } : { display: 'block' }}>
