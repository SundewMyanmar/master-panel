...
class LoginPage extends React.Component {
    ...
    onLoginButtonClick = async () => {
        ...
        if (!window.navigator.onLine) {
            this.setState({ showError: true, errorMessage: 'Please check your internet connection and try again.' });
            return;
        }
        ...
        try {
            ...
            if (data) {
                ...
                if (data.roles.map(t => t.id).indexOf(USER_ROLES.SUPER_USER) === -1 && data.roles.map(t => t.id).indexOf(USER_ROLES.ADMIN) === -1) {
                    this.setState({
                        showError: true,
                        showLoading: false,
                ...
                <Grid className={classes.container} container spacing={4} alignItems="center" justify="center">
                    <Grid style={{ padding: '22px' }} className={classes.cardBox} item xs={12} sm={8} md={6} lg={4}>
                        ...
                        <Grid item>
                            ...
                            <Typography
                                style={{ color: text.main, textAlign: 'left', margin: '4px 0 12px 10px', fontSize: '14px' }}
                                variant="subtitle1"
                            >
                                Not register yet,{' '}
                                <a style={{ color: secondary.main, textDecoration: 'none' }} rel="noopener noreferrer" href="#/register">
                                    create new account
                                </a>
                                ?
                            </Typography>
                            ...
                            <Typography
                                style={{ color: text.main, textAlign: 'center', margin: '0px 0px 8px 0px', fontSize: '14px' }}
                                variant="subtitle1"
                            >
